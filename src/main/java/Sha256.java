...
public class Sha256 {
    ...
    static String dataToHash ;
    public static void main(String args[]) {
    	 	
//    	byte[] resByte = Sha256.hash();
//    	String res =bytesToHex(resByte);
//        System.out.println("output _ "+res);
        int prefix =6;
        previousHash = "0035ea28e7373f08d81b387a758c4b9bac7003ed048f4ca1238bd132146020e9";
        data = "This is a test blockchain";
        nonce =1792309;
        showTime();
        hash= mineBlock(prefix); 
    ...
    public static byte[] hash() {
    	...
    	dataToHash = previousHash  
	 	...
	 	message = dataToHash.getBytes(StandardCharsets.UTF_8);
        ...
        for (int i = 0, n = words.length / 16; i < n; ++i) {
            ...
            System.out.println("words cc "+i);
            // initialize W from the block's words
            System.arraycopy(words, i * 16, W, 0, 16);
            System.out.println("W byte arrays :: "+bytesToHex(toByteArray_(W)));
            for (int t = 16; t < W.length; ++t) {
                W[t] = smallSig1(W[t - 2]) + W[t - 7] + smallSig0(W[t - 15]) + W[t - 16];
            }


            System.out.println("H byte arrays :: "+bytesToHex(toByteArray_(H)));
            // let TEMP = H

            System.arraycopy(H, 0, TEMP, 0, H.length);


            System.out.println("Temp byte arrays :: "+bytesToHex(toByteArray_(TEMP)));
	        ...
	        System.out.println("bytes H  "+H[0]);
	        System.out.println("bytes  "+TEMP[0]);
	        ...
	        System.out.println("bytes  H "+H[0]);
	        System.out.println("bytes  H "+H[1]);
	        System.out.println("bytes  H "+H[2]);
//            H = TEMP;
	        System.out.println("H byte arrays :: "+bytesToHex(toByteArray_(H)));
	          
        }
//        System.out.println("Real out "+bytesToHex(toByteArray(H)));
    ...
    private static byte[] toByteArray(int[] ints) {
        ...
        for (int i : ints) {
            buf.putInt(i);
            System.out.println("toByteArray "+i);
        }

        return buf.array();
    }
    private static byte[] toByteArray_(int[] ints) {
        ByteBuffer buf = ByteBuffer.allocate(ints.length * Integer.BYTES);
        for (int i : ints) {
            buf.putInt(i);
//            System.out.println("toByteArray "+i);
